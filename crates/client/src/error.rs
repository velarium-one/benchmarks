...
impl Error {
    pub(crate) fn contract(message: impl Into<String>) -> Self {
        Self { code: 4, stage: "client".into(), message: message.into(), source_pc: None,
...
pub(crate) unsafe fn bytes<'a>(view: Bytes) -> Result<&'a [u8]> {
    ...
    if len > isize::MAX as usize { return Err(Error::contract("byte extent exceeds slice limit")); }
    if len == 0 { return Ok(&[]); }
    if view.ptr.is_null() { return Err(Error::contract("nonempty byte view is null")); }
...
impl Engine {
    pub(crate) fn finish(&self, status: u64, error: *mut ErrorHandle) -> Result<()> {
        ...
        impl Drop for OwnedError<'_> {
            fn drop(&mut self) { unsafe { (self.0.api().release_error)(self.1); } }
        }
        let _record = OwnedError(self, error);
        if status == 0 {
            return if error.is_null() { Ok(()) } else { Err(Error::contract("success returned an error record")) };
        }
        if error.is_null() { return Err(Error::contract(format!("status {status} omitted error record"))); }
        let mut view = std::mem::MaybeUninit::<ErrorView>::uninit();
        let viewed = unsafe { (self.api().error_view)(error, view.as_mut_ptr()) };
        if viewed != 0 { return Err(Error::contract("cannot read engine error record")); }
        ...
        Err(Error { code: view.code, stage: text(view.stage)?, message: text(view.message)?,
            source_pc: match view.source_pc_present {
                0 => None, 1 => Some(view.source_pc),
